...
function SvcF({item,D,save}){
  const[f,setF]=useState(item||{name:'',duration:60,price:0,category:'Hammam',description:'',active:true,allowedResources:[]});
  ...
  const s=()=>{
    if(!f.name)return;
    const arr=item?(D.services||[]).map(x=>x.id===item.id?{...f,id:item.id,price:+f.price,duration:+f.duration}:x):[...(D.services||[]),{...f,id:nid(D.services||[]),price:+f.price,duration:+f.duration}];
    save('services',arr);
  };
...
function ApptF({item,D,save}){
  ...
  const selSvcAuto=(i,e)=>{
    ...
    if(!s)return;
    setRows(r=>{
      const upd=r.map((x,j)=>j===i?{...x,serviceId:s.id,serviceName:s.name,price:s.price,duration:s.duration}:x);
