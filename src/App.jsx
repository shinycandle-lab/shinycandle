...
const DELETE_CODE = '1971'; // código para eliminar movimientos de caja
...
function autoAssignResource(service, date, time, duration, appointments, resources){
  ...
  const allowed=service.allowedResources||(()=>{
    const def={'Hammam':['zona','cabina'],'Cabello':['silla','cabina'],'Masaje':['cabina'],'Facial':['cabina'],'Especial':['cabina'],'Cuerpo':['cabina']};
    return null; // sin filtro → usar todos activos
  })();
...
function CierreModal({appt,D,onClose,onDone,multiCount=1,multiTotal=0}){
  ...
  const basePrice=multiCount>1?multiTotal:appt.price;
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,backdropFilter:'blur(4px)'}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:A.sf,border:`1px solid ${A.bd}`,borderRadius:20,padding:26,width:520,maxHeight:'92vh',overflowY:'auto'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:18}}>
          <div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:19}}>{multiCount>1?`Cobrar ${multiCount} citas`:'Cerrar Cita'}</div>
...
function MultiCobroModal({appt,allAppts,onCobrarSingle,onCobrarMulti,onClose}){
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,backdropFilter:'blur(4px)',padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:ADM.sf,border:`1px solid ${ADM.bd}`,borderRadius:20,padding:26,width:480,maxHeight:'88vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.4)'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:18}}>
          <div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:19,color:ADM.text}}>Cobro de citas</div>
            <div style={{fontSize:12,color:ADM.muted,marginTop:2}}>{appt.clientName} tiene varias citas hoy</div>
          </div>
        ...
        <div style={{background:`${G}10`,border:`1px solid ${G}30`,borderRadius:12,padding:'12px 14px',marginBottom:12}}>
          <div style={{fontSize:10,color:G,letterSpacing:1,textTransform:'uppercase',marginBottom:6,fontWeight:600}}>Cita principal (siempre incluida)</div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div><div style={{fontWeight:600,fontSize:14,color:ADM.text}}>{appt.serviceName}</div><div style={{fontSize:12,color:ADM.muted,marginTop:2}}>{appt.time} · {appt.staffName}</div></div>
            <span style={{fontFamily:"'Playfair Display',serif",fontSize:16,color:G,fontWeight:700}}>{fmt(appt.price)}</span>
          </div>
        </div>
        <div style={{fontSize:12,color:ADM.muted,marginBottom:8,fontWeight:600}}>Selecciona otras citas de hoy para cobrar juntas:</div>
        {cobrables.map(a=>{
          const sel=selected.includes(a.id);
          return(
            <div key={a.id} onClick={()=>toggle(a.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',border:`2px solid ${sel?G:ADM.bd}`,borderRadius:12,background:sel?`${G}10`:ADM.sf2,cursor:'pointer',marginBottom:8,transition:'all .15s'}}>
              ...
              <div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:13,color:sel?G:ADM.text}}>{a.serviceName}</div><div style={{fontSize:11,color:ADM.muted,marginTop:2}}>{a.time} · {a.staffName}</div></div>
...
function AdminAgenda({D,commit,setModal,ask}){
  ...
  const cobrarMulti=(appts,{method,sold,extraSvcs,tip,notes,total,giftCard})=>{
    ...
    const txs=[];let offset=0;
    appts.forEach((a,i)=>{txs.push({id:id0+offset,date:today,type:'income',category:'Servicios',description:`${a.serviceName} — ${a.clientName}`,amount:a.price,method,staffId:a.staffId});offset++;});
    if(tip>0){txs.push({id:id0+offset,date:today,type:'income',category:'Servicios',description:`Propina — ${appts[0].clientName}`,amount:tip,method,staffId:appts[0].staffId});offset++;}
    sold.forEach((it,i)=>{txs.push({id:id0+offset+i,date:today,type:'income',category:'Productos',description:`${it.name} x${it.qty} — ${appts[0].clientName}`,amount:it.price*it.qty,method,staffId:appts[0].staffId});});
    ...
    const clientId=appts[0].clientId;
    const updC=(D.clients||[]).map(c=>c.id===clientId?{...c,visits:(c.visits||0)+appts.length,totalSpent:(c.totalSpent||0)+total,lastVisit:today}:c);
  ...
  return(<div style={{display:'flex',gap:16}}>
    ...
    <div style={{flex:1,minWidth:0}}>
      ...
      appts.map(a=><ABox key={a.id} sx={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap',borderLeft:`3px solid ${SC[a.status]||A.bd}`,paddingLeft:16,marginBottom:10}} ch={<>
        ...
        <div style={{flex:1,minWidth:120}}><div style={{fontWeight:600,fontSize:14}}>{a.clientName}</div><div style={{fontSize:12,color:A.muted,marginTop:2}}>{a.serviceName}</div>{a.notes&&<div style={{fontSize:11,color:A.gold,marginTop:2,fontStyle:'italic'}}>📝 {a.notes}</div>}<div style={{fontSize:11,color:A.muted,marginTop:2}}>👤 {a.staffName}{a.resourceName&&<span style={{color:A.gold,marginLeft:8}}>🛏 {a.resourceName}</span>}{a.source==='web'&&<span style={{color:'#9B7FD4',marginLeft:6}}>🌐 web · {a.email}</span>}</div></div>
        <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:6}}>
          ...
          <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
            ...
            {(a.status==='confirmed'||a.status==='pending')&&<ABtn sz="sm" v="success" ch="💳 Cobrar" onClick={()=>{
              const otras=(D.appointments||[]).filter(x=>x.clientId===a.clientId&&x.date===a.date&&(x.status==='confirmed'||x.status==='pending')&&x.id!==a.id);
              if(otras.length>0)setMultiSelector(a);else setCierre(a);
            }}/>}
    ...
    {cierreMulti&&<CierreModal appt={cierreMulti[0]} D={D} onClose={()=>{setCierreMulti(null);setMultiSelector(null);}} onDone={(data)=>cobrarMulti(cierreMulti,data)} multiCount={cierreMulti.length} multiTotal={cierreMulti.reduce((s,a)=>s+a.price,0)}/>}
...
function AdminServicios({D,commit,setModal,ask}){
  ...
  return(<div><AHead title="Servicios" action={<ABtn ch={<><Plus size={13}/>Nuevo</>} onClick={()=>setModal({entity:'services',item:null})}/>}/>{cats.map(cat=><div key={cat} style={{marginBottom:22}}><div style={{fontSize:10,color:A.gold,letterSpacing:2,marginBottom:10,textTransform:'uppercase'}}>{cat}</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:12}}>{(D.services||[]).filter(s=>s.category===cat).map(s=><ABox key={s.id} sx={{opacity:s.active?1:0.5}} ch={<><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><div style={{fontWeight:600,fontSize:13,flex:1}}>{s.name}
                  {s.webVisible===false&&<span style={{marginLeft:6,fontSize:9,background:`${A.gold}20`,color:A.gold,border:`1px solid ${A.gold}40`,borderRadius:10,padding:'1px 7px',fontWeight:700,letterSpacing:.5,textTransform:'uppercase',verticalAlign:'middle'}}>fuera de carta</span>}
                </div><div style={{display:'flex',gap:4}}><ABtn sz="sm" v="ghost" ch={<Edit2 size={10}/>} onClick={()=>setModal({entity:'services',item:s})}/><ABtn sz="sm" v="danger" ch={<Trash2 size={10}/>} onClick={()=>ask(()=>commit({...D,services:(D.services||[]).filter(x=>x.id!==s.id)}),'¿Eliminar este servicio?')}/></div></div><div style={{fontSize:11,color:A.muted,marginBottom:10,lineHeight:1.5}}>{s.description}</div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div>
                    {(s.variants||[]).length>0
                      ?<div>{s.variants.map(v=><div key={v.id} style={{fontSize:12,color:A.muted,marginBottom:2}}><span style={{color:A.gold,fontWeight:700}}>{fmt(v.price)}</span> <span style={{fontSize:11}}>{v.name}</span></div>)}</div>
                      :<span style={{color:A.gold,fontFamily:"'Playfair Display',serif",fontSize:16,fontWeight:700}}>{fmt(s.price)}</span>
                    }
                    <span style={{fontSize:11,color:A.muted,marginLeft:(s.variants||[]).length>0?0:6}}>{s.duration}min</span>
                  </div></div><ABtn sz="sm" v={s.active?'secondary':'ghost'} ch={s.active?'Activo':'Inactivo'} onClick={()=>commit({...D,services:(D.services||[]).map(x=>x.id===s.id?{...x,active:!x.active}:x)})}/></div></>}/>)}</div></div>)}</div>);
}

function AdminProductos({D,commit,setModal,ask}){
  return(<div><AHead title="Productos" action={<ABtn ch={<><Plus size={13}/>Nuevo</>} onClick={()=>setModal({entity:'products',item:null})}/>}/><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:14}}>{(D.products||[]).map(p=><ABox key={p.id} ch={<>
              {p.image
                ?<div style={{height:100,borderRadius:10,overflow:'hidden',marginBottom:10,background:A.sf2}}><img src={p.image} alt={p.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/></div>
                :<div style={{fontSize:28,textAlign:'center',marginBottom:10,background:A.sf2,borderRadius:10,padding:'14px 0'}}>{EMOJIS[p.category]||'📦'}</div>
              }<div style={{fontWeight:600,fontSize:13,marginBottom:3}}>{p.name}</div><div style={{fontSize:11,color:A.muted,marginBottom:10}}>{p.description}</div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}><span style={{color:A.gold,fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700}}>{fmt(p.price)}</span><span style={{fontSize:11,background:p.stock>5?`${A.teal}15`:`${A.terra}15`,color:p.stock>5?A.teal:A.terra,padding:'2px 8px',borderRadius:20}}>Stock: {p.stock}</span></div><div style={{display:'flex',gap:6}}><ABtn sz="sm" v="secondary" ch={<><Edit2 size={10}/>Editar</>} onClick={()=>setModal({entity:'products',item:p})} sx={{flex:1}}/><ABtn sz="sm" v="danger" ch={<Trash2 size={10}/>} onClick={()=>ask(()=>commit({...D,products:(D.products||[]).filter(x=>x.id!==p.id)}),'¿Eliminar este producto?')}/></div></>}/>)}</div></div>);
}
...
function AdminCaja({D,commit,setModal}){
  ...
  const [delTx,setDelTx]=useState(null);     // {id} de la tx a eliminar
  const [delCode,setDelCode]=useState('');
  const [delErr,setDelErr]=useState(false);
  const confirmDelTx=()=>{
    if(delCode===DELETE_CODE){
      commit({...D,transactions:(D.transactions||[]).filter(t=>t.id!==delTx)});
      setDelTx(null);setDelCode('');setDelErr(false);
    } else {
      setDelErr(true);setTimeout(()=>setDelErr(false),1500);
    }
  };
  const hoy=tod();
  ...
  return(<div>
    ...
    {delTx&&(
      <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,backdropFilter:'blur(4px)'}}>
        <div style={{background:A.sf,border:`1px solid ${A.bd}`,borderRadius:16,padding:28,width:340,textAlign:'center',boxShadow:'0 20px 60px rgba(0,0,0,0.4)'}}>
          <div style={{fontSize:32,marginBottom:12}}>🗑</div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:A.text,marginBottom:6}}>Eliminar movimiento</div>
          <div style={{fontSize:13,color:A.muted,marginBottom:18,lineHeight:1.6}}>Introduce el código de autorización para eliminar este movimiento. Esta acción no se puede deshacer.</div>
          <input type="password" value={delCode} onChange={e=>{setDelCode(e.target.value);setDelErr(false);}} onKeyDown={e=>e.key==='Enter'&&confirmDelTx()} placeholder="Código" autoFocus style={{width:'100%',padding:'12px',background:A.sf2,border:`2px solid ${delErr?A.terra:delCode?A.gold:A.bd}`,borderRadius:10,color:A.text,fontSize:18,fontFamily:"'Nunito',sans-serif",outline:'none',textAlign:'center',letterSpacing:4,marginBottom:8,transition:'border-color .2s'}}/>
          {delErr&&<div style={{fontSize:12,color:A.terra,marginBottom:8}}>Código incorrecto</div>}
          <div style={{display:'flex',gap:10,marginTop:10}}>
            <button onClick={()=>{setDelTx(null);setDelCode('');setDelErr(false);}} style={{flex:1,padding:'10px',border:`1px solid ${A.bd}`,borderRadius:10,background:'transparent',color:A.muted,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>Cancelar</button>
            <button onClick={confirmDelTx} style={{flex:1,padding:'10px',border:'none',borderRadius:10,background:A.terra,color:'#fff',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>Eliminar</button>
          </div>
        </div>
      </div>
    )}
    <div style={{display:'flex',gap:10,marginBottom:16}}>
      <div style={{flex:1,background:aperturaHoy?`${A.teal}12`:`${A.terra}12`,border:`1px solid ${aperturaHoy?A.teal:A.terra}33`,borderRadius:12,padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
        <div>
          <div style={{fontSize:10,color:A.muted,letterSpacing:1,textTransform:'uppercase',marginBottom:3}}>Estado de caja hoy</div>
          <div style={{fontSize:15,fontWeight:700,color:aperturaHoy?A.teal:A.terra}}>{aperturaHoy?`🟢 Abierta desde ${aperturaHoy.hora}`:'🔴 Sin abrir'}</div>
          {aperturaHoy&&<div style={{fontSize:11,color:A.muted,marginTop:2}}>Saldo inicial: {fmt(aperturaHoy.fondoTotal||0)}{aperturaHoy.nota&&` · ${aperturaHoy.nota}`}</div>}
          {cierreHoy&&<div style={{fontSize:11,color:A.muted,marginTop:2}}>Cerrada a las {cierreHoy.hora}</div>}
        </div>
        <div style={{display:'flex',gap:8}}>
          {!aperturaHoy&&<ABtn ch="🔓 Abrir caja" onClick={()=>setAperturaOpen(true)} v="secondary" sz="sm"/>}
          {aperturaHoy&&!cierreHoy&&<ABtn ch="💰 Cerrar caja" onClick={()=>setArqueo(true)} v="secondary" sz="sm"/>}
        </div>
      </div>
    </div>
    <AHead title="Caja & Contabilidad" action={<div style={{display:'flex',gap:8}}><ABtn ch={<><Plus size={13}/>Movimiento</>} onClick={()=>setModal({entity:'transactions',item:null})}/>{!aperturaHoy?<ABtn ch="🔓 Abrir caja" onClick={()=>setAperturaOpen(true)} v="secondary"/>:(!cierreHoy&&<ABtn ch="💰 Cerrar caja" onClick={()=>setArqueo(true)} v="secondary"/>)}</div>}/><div style={{display:'flex',gap:14,marginBottom:18,flexWrap:'wrap'}}><AKpi label="Ingresos" value={fmt(inc)} icon={TrendingUp} color={A.teal}/><AKpi label="Gastos" value={fmt(exp)} icon={DollarSign} color={A.terra}/><AKpi label="Balance" value={fmt(inc-exp)} icon={Star} color={inc>=exp?A.gold:A.terra}/></div><div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}><div style={{display:'flex',gap:6}}>{['all','income','expense'].map(f=><ABtn key={f} sz="sm" v={filter===f?'primary':'ghost'} ch={f==='all'?'Todo':f==='income'?'Ingresos':'Gastos'} onClick={()=>setFilter(f)}/>)}</div><div style={{display:'flex',gap:6}}>{[['today','Hoy'],['week','Semana'],['month','Mes'],['all','Todo']].map(([k,l])=><ABtn key={k} sz="sm" v={dr===k?'secondary':'ghost'} ch={l} onClick={()=>setDr(k)}/>)}</div></div><ABox ch={fl.length===0?<div style={{textAlign:'center',padding:'30px 0',color:A.muted}}>Sin movimientos</div>:fl.map((t,i)=><div key={t.id} style={{display:'flex',alignItems:'center',gap:14,padding:'10px 0',borderBottom:i<fl.length-1?`1px solid ${A.bd}`:'none'}}><div style={{width:34,height:34,borderRadius:9,background:t.type==='income'?`${A.teal}20`:`${A.terra}20`,display:'flex',alignItems:'center',justifyContent:'center',color:t.type==='income'?A.teal:A.terra,fontSize:16,flexShrink:0}}>{t.type==='income'?'↑':'↓'}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.description}</div><div style={{fontSize:11,color:A.muted}}>{t.category} · {t.method} · {t.date}</div></div><div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
              <div style={{fontSize:14,fontWeight:700,color:t.type==='income'?A.teal:A.terra}}>{t.type==='income'?'+':'-'}{fmt(t.amount)}</div>
              <button onClick={()=>{setDelTx(t.id);setDelCode('');}} title="Eliminar movimiento" style={{background:'none',border:`1px solid ${A.terra}33`,borderRadius:7,padding:'4px 6px',cursor:'pointer',color:A.terra,display:'flex',alignItems:'center',opacity:0.6}} onMouseOver={e=>e.currentTarget.style.opacity='1'} onMouseOut={e=>e.currentTarget.style.opacity='0.6'}><Trash2 size={12}/></button>
            </div></div>)}/></div>);
  if(arqueo)return <ArqueoModal D={D} commit={commit} onClose={()=>setArqueo(false)}/>;
}
...
function SvcF({item,D,save}){
  ...
  const s=()=>{
    if(!f.name)return;
    const clean={...f,price:+f.price,duration:+f.duration,variants:(f.variants||[]).filter(v=>v.name.trim()).map(v=>({...v,price:+v.price}))};
    const arr=item?(D.services||[]).map(x=>x.id===item.id?{...clean,id:item.id}:x):[...(D.services||[]),{...clean,id:nid(D.services||[])}];
    save('services',arr);
  };
  ...
  return(<div>
    ...
    <div style={{display:'flex',gap:10}}>
      <AFld label="Precio €" ch={<input style={inp} type="number" value={f.price} onChange={F('price')}/>}/>
      <AFld label="Duración min" ch={<input style={inp} type="number" value={f.duration} onChange={F('duration')}/>}/>
    </div>
    ...
    <div style={{marginBottom:12}}>
      ...
      <div style={{fontSize:11,color:A.muted,marginBottom:10,lineHeight:1.6}}>
        Añade variantes si el precio varía (ej. por longitud de cabello). Si no añades ninguna, se usará el precio base.
      </div>
      {(f.variants||[]).map(vr=>(
        <div key={vr.id} style={{display:'flex',gap:8,alignItems:'center',marginBottom:8}}>
          <input value={vr.name} onChange={e=>updVariant(vr.id,'name',e.target.value)} placeholder="Ej. Cabello corto, liso, con extensiones..." style={{...inp,flex:2}}/>
          <div style={{display:'flex',alignItems:'center',gap:4,flex:1}}>
            <input type="number" value={vr.price} onChange={e=>updVariant(vr.id,'price',e.target.value)} placeholder="Precio" style={{...inp,textAlign:'right'}} min="0"/>
            <span style={{fontSize:12,color:A.muted,flexShrink:0}}>€</span>
          </div>
          <button onClick={()=>delVariant(vr.id)} style={{background:'none',border:`1px solid ${A.terra}44`,borderRadius:7,padding:'6px 8px',cursor:'pointer',color:A.terra,display:'flex',alignItems:'center'}}><X size={12}/></button>
        </div>
      ))}
      <button onClick={addVariant} style={{width:'100%',padding:'8px',border:`2px dashed ${A.gold}44`,borderRadius:9,background:`${A.gold}08`,color:A.gold,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
        <Plus size={12}/> Añadir variante
      </button>
      {(f.variants||[]).length>0&&(
        <div style={{marginTop:8,fontSize:11,color:A.muted,background:A.sf2,borderRadius:8,padding:'8px 12px'}}>
          Precio base ({fmt(+f.price||0)}) se usará como referencia. Las variantes mostrarán sus propios precios.
        </div>
      )}
    </div>
    <AFld label="🛏 Recursos permitidos" ch={
      <div>
        <div style={{fontSize:11,color:A.muted,marginBottom:8}}>Selecciona qué recursos puede usar este servicio. El sistema asignará el primero disponible.</div>
        {resources.length===0&&<div style={{fontSize:12,color:A.muted}}>No hay recursos creados. Añádelos en la pestaña Recursos.</div>}
        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {resources.filter(r=>r.active).map(r=>{
            const sel=(f.allowedResources||[]).includes(r.id);
            return(
              <button key={r.id} onClick={()=>toggleRes(r.id)} style={{padding:'7px 13px',border:`2px solid ${sel?r.color||A.gold:A.bd}`,borderRadius:20,background:sel?`${r.color||A.gold}18`:'transparent',color:sel?r.color||A.gold:A.muted,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:6}}>
                {sel&&<Check size={11}/>}{r.name}
              </button>
            );
          })}
        </div>
        {(f.allowedResources||[]).length===0&&resources.length>0&&<div style={{fontSize:11,color:A.terra,marginTop:6}}>⚠️ Sin recursos asignados — no se podrán autoasignar citas para este servicio</div>}
      </div>
    }/>
    ...
    <div style={{marginBottom:16,background:f.webVisible!==false?`${A.teal}10`:`${A.gold}10`,border:`1px solid ${f.webVisible!==false?A.teal:A.gold}33`,borderRadius:10,padding:'12px 14px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:f.webVisible!==false?A.teal:A.gold}}>{f.webVisible!==false?'🌐 Visible en la web':'🔒 Fuera de carta (solo interno)'}</div>
          <div style={{fontSize:11,color:A.muted,marginTop:3}}>{f.webVisible!==false?'Las clientas pueden verlo y reservarlo online':'Solo disponible desde el panel de gestión'}</div>
        </div>
        <button onClick={()=>setF(p=>({...p,webVisible:p.webVisible===false}))} style={{padding:'8px 14px',border:`2px solid ${f.webVisible!==false?A.teal:A.gold}`,borderRadius:20,background:f.webVisible!==false?`${A.teal}15`:`${A.gold}15`,color:f.webVisible!==false?A.teal:A.gold,cursor:'pointer',fontSize:12,fontWeight:700,fontFamily:'inherit',flexShrink:0}}>
          {f.webVisible!==false?'Ocultar de web':'Mostrar en web'}
        </button>
      </div>
    </div>
...
function ProdF({item,D,save}){
  ...
  return(<div>
    {/* Imagen del producto */}
    <AFld label="Imagen del producto" ch={
      <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
        <div style={{width:80,height:80,borderRadius:12,border:`2px dashed ${f.image?A.gold:A.bd}`,background:A.sf2,overflow:'hidden',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}} onClick={uploadImg}>
          {f.image
            ?<img src={f.image} alt="producto" style={{width:'100%',height:'100%',objectFit:'cover'}}/>
            :<div style={{textAlign:'center',color:A.muted}}><div style={{fontSize:24,marginBottom:4}}>📷</div><div style={{fontSize:10}}>Subir</div></div>
          }
        </div>
        <div style={{flex:1}}>
          <ABtn ch="📷 Elegir imagen" onClick={uploadImg} v="ghost" sz="sm" sx={{marginBottom:6}}/>
          {f.image&&<ABtn ch={<><X size={11}/>Quitar</>} onClick={()=>setF(p=>({...p,image:null}))} v="danger" sz="sm"/>}
          <div style={{fontSize:11,color:A.muted,marginTop:6}}>JPG, PNG o WebP · Máx. 2MB</div>
        </div>
      </div>
    }/>
    <AFld label="Nombre" ch={<input style={inp} value={f.name} onChange={F('name')}/>}/>
...
function ApptF({item,D,save}){
  ...
  const doAutoResource=(rows,i)=>{
    const r=rows[i];
    if(!r.date||!r.time)return rows;
    const svc=(D.services||[]).find(s=>s.id===+r.serviceId);
    if(!svc)return rows;
    const res=autoAssignResource(svc,r.date,r.time,r.duration||svc.duration,D.appointments||[],D.resources||[]);
    return rows.map((x,j)=>j===i?{...x,resourceId:res?.id||null,resourceName:res?.name||null}:x);
  };
  const selSvcAuto=(i,e)=>{
    const s=(D.services||[]).find(s=>s.id===+e.target.value);
    if(!s)return;
    const hasVariants=(s.variants||[]).length>0;
    const defVariant=hasVariants?s.variants[0]:null;
    setRows(r=>{
      const upd=r.map((x,j)=>j===i?{
        ...x,serviceId:s.id,serviceName:s.name,
        price:defVariant?defVariant.price:s.price,
        duration:s.duration,
        variantId:defVariant?.id||null,
        variantName:defVariant?.name||null,
      }:x);
      return doAutoResource(upd,i);
    });
  };
  const selVariantAdmin=(i,vid)=>{
    setRows(r=>r.map((x,j)=>{
      if(j!==i)return x;
      const svc=(D.services||[]).find(s=>s.id===+x.serviceId);
      const vr=(svc?.variants||[]).find(v=>v.id===+vid);
      return vr?{...x,variantId:vr.id,variantName:vr.name,price:vr.price}:{...x,variantId:null,variantName:null,price:svc?.price||x.price};
    }));
  };
  const updAuto=(i,k,v)=>{
    setRows(r=>{
      const upd=r.map((x,j)=>j===i?{...x,[k]:v}:x);
      return(k==='date'||k==='time')?doAutoResource(upd,i):upd;
    });
  };
    ...
    <div>
      {rows.map((f,i)=>(
        <div key={i} style={{background:i%2===0?ADM.sf2:'transparent',borderRadius:10,padding:12,marginBottom:12,border:`1px solid ${ADM.bd}`,position:'relative'}}>
          ...
          {/* Variante de precio */}
          {(()=>{const svc=(D.services||[]).find(s=>s.id===+f.serviceId);const variants=svc?.variants||[];if(!variants.length)return null;return(
            <AFld label="Variante / tipo" ch={
              <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                {variants.map(v=>(
                  <button key={v.id} onClick={()=>selVariantAdmin(i,v.id)} style={{padding:'7px 13px',border:`2px solid ${f.variantId===v.id?ADM.gold:ADM.bd}`,borderRadius:20,background:f.variantId===v.id?`${ADM.gold}15`:'transparent',color:f.variantId===v.id?ADM.gold:ADM.muted,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:5}}>
                    {f.variantId===v.id&&<Check size={11}/>}{v.name} — {fmt(v.price)}
                  </button>
                ))}
              </div>
            }/>
          ...
          {f.resourceId&&<div style={{background:ADM.sf2,borderRadius:8,padding:'8px 12px',marginBottom:10,display:'flex',alignItems:'center',gap:8,border:`1px solid ${ADM.gold}33`}}><span style={{fontSize:14}}>🛏</span><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:ADM.gold}}>{f.resourceName}</div><div style={{fontSize:10,color:ADM.muted}}>Asignado automáticamente</div></div></div>}
          {f.serviceId&&f.date&&f.time&&!f.resourceId&&<div style={{background:`${ADM.terra}10`,borderRadius:8,padding:'8px 12px',marginBottom:10,fontSize:12,color:ADM.terra,border:`1px solid ${ADM.terra}30`}}>⚠️ Sin recursos disponibles en este horario — configura recursos en el servicio</div>}
...
function AperturaModal({D,commit,onClose}){
  ...
  const totalFondo=Object.values(fondos).reduce((s,v)=>s+(parseFloat(v)||0),0);
  const MROWS=[['💵','Efectivo','efectivo'],['💳','Tarjeta','tarjeta'],['📱','Bizum','bizum'],['🏦','Transferencia','transferencia']];
  const abrir=()=>{
    const apertura={id:nid(D.aperturas||[]),fecha:t,hora,ts:Date.now(),
      ...
      fondoTotal:totalFondo,nota:nota.trim()};
    commit({...D,aperturas:[...(D.aperturas||[]),apertura]});
    onClose();
  };
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,backdropFilter:'blur(4px)'}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:A.sf,border:`1px solid ${A.bd}`,borderRadius:20,padding:28,width:500,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.4)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20}}>
          <div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:A.text}}>🔓 Apertura de Caja</div>
            <div style={{fontSize:12,color:A.muted,marginTop:3}}>{new Date().toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'})} · {hora}</div>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',color:A.muted,cursor:'pointer'}}><X size={18}/></button>
        </div>
        <div style={{fontSize:12,color:A.muted,marginBottom:14,lineHeight:1.6}}>Registra el saldo disponible en cada método al iniciar el turno.</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 140px',gap:12,padding:'6px 0',marginBottom:4}}>
          ...
          <div style={{fontSize:10,color:A.muted,letterSpacing:1,textTransform:'uppercase',textAlign:'center'}}>Saldo inicial</div>
        </div>
        ...
        <div style={{background:`${A.gold}10`,border:`1px solid ${A.gold}25`,borderRadius:10,padding:'12px 16px',margin:'16px 0',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <span style={{fontSize:13,color:A.muted}}>Saldo total de apertura</span>
          <span style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:A.gold,fontWeight:700}}>{fmt(totalFondo)}</span>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:20}}>
          {visProd.map(p=>(
            <div key={p.id} style={{background:WEB.sf,border:`2px solid ${added===p.id?WEB.gold:WEB.bd}`,borderRadius:18,padding:22,transition:'all .25s'}}>
              {p.image
                ?<div style={{height:110,borderRadius:12,overflow:'hidden',marginBottom:14,background:WEB.light}}><img src={p.image} alt={p.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/></div>
  ...
  return(
    <div style={{background:WEB.sf,border:`1px solid ${WEB.bd}`,borderRadius:18,padding:24,transition:'transform .2s,box-shadow .2s'}} onMouseOver={e=>{e.currentTarget.style.transform='translateY(-3px)';e.currentTarget.style.boxShadow='0 10px 32px rgba(0,0,0,0.07)';}} onMouseOut={e=>{e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='';}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
        <div>
          {(s.variants||[]).length>0
            ?<div><span style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:WEB.gold}}>desde {fmt(Math.min(...s.variants.map(v=>v.price)))}</span><span style={{fontSize:12,color:WEB.muted,marginLeft:6}}>{s.duration}min</span></div>
...
function BookingModal({D,commit,onClose}){
  ...
  const confirm=()=>{
    ...
    const newAppt={
      ...
      serviceName:selSvcs.map(sv=>sv.name+(getVariantName(sv)?` (${getVariantName(sv)})`:'')).join(' + '),
        ...
        <div style={{padding:'16px 24px 28px'}}>
          ...
          {step===1&&<div>
            ...
            {selSvcs.length>0&&(
              <div style={{background:`${G}10`,border:`1px solid ${G}25`,borderRadius:12,padding:'12px 14px',marginBottom:14}}>
                {selSvcs.map(sv=><div key={sv.id} style={{display:'flex',justifyContent:'space-between',fontSize:12,color:WEB.muted,marginBottom:3}}><span>{sv.name}{getVariantName(sv)&&<span style={{color:WEB.gold,marginLeft:4}}>· {getVariantName(sv)}</span>}</span><span>{fmt(getEffectivePrice(sv))}</span></div>)}
          ...
          {step===5&&<div style={{textAlign:'center',padding:'10px 0'}}>
            ...
            <div style={{background:WEB.light,borderRadius:12,padding:'12px 16px',marginBottom:16,textAlign:'left'}}>
              {selSvcs.map(sv=><div key={sv.id} style={{fontSize:13,color:WEB.muted,marginBottom:3}}>· {sv.name}{getVariantName(sv)&&<span style={{color:WEB.gold}}> · {getVariantName(sv)}</span>}</div>)}
