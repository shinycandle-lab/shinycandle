...
const dbWrite = async (d) => {
  ...
  // Copia de seguridad local automática en cada guardado
  try {
...
function BackupPanel({D,commit}){
    ...
    const fi=document.createElement('input');fi.type='file';fi.accept='.json';
      ...
      reader.onload=ev=>{
        try{
          ...
          const d=parsed.data||parsed; // compatible con ambos formatos
          if(!d.services||!d.clients){setRestoreMsg('❌ Archivo no válido — no parece un backup de ShinyCandle');setRestoreOk(false);return;}
          commit(d);
          setRestoreMsg(`✅ Datos restaurados correctamente (${new Date(parsed.exportedAt||Date.now()).toLocaleDateString('es-ES')})`);
          setRestoreOk(true);
          setTimeout(()=>{setRestoreMsg('');setRestoreOk(null);},4000);
  ...
  const localBackup=()=>{
    try{
      ...
      const fecha=new Date(parsed.ts).toLocaleString('es-ES');
      if(window.confirm&&typeof window.confirm==='function'){
        // En algunos entornos confirm no funciona, asumimos sí
      }
      commit(parsed.data);
      setRestoreMsg(`✅ Restaurado desde backup local del ${fecha}`);
      setRestoreOk(true);
      setTimeout(()=>{setRestoreMsg('');setRestoreOk(null);},4000);
  ...
  const localBackupInfo=()=>{
    try{
      ...
      if(!b)return null;
      const parsed=JSON.parse(b);
      return new Date(parsed.ts).toLocaleString('es-ES');
    }catch(e){return null;}
  };

  const backupDate=localBackupInfo();
  ...
  return(
    <ABox sx={{marginBottom:16,border:`1px solid ${A.teal}33`}} ch={<>
      ...
      <div style={{fontSize:12,color:A.muted,marginBottom:16,lineHeight:1.7}}>
        Los datos se guardan automáticamente en Supabase (nube) y en el navegador (local).<br/>
        Exporta un backup JSON siempre que quieras tener una copia en tu ordenador.
      </div>

      <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:12}}>
        <button onClick={exportBackup} style={{flex:1,minWidth:160,padding:'11px 14px',background:`${A.teal}15`,border:`1px solid ${A.teal}44`,borderRadius:10,color:A.teal,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
          📥 Exportar backup JSON
        </button>
        <button onClick={importBackup} style={{flex:1,minWidth:160,padding:'11px 14px',background:`${A.gold}10`,border:`1px solid ${A.gold}44`,borderRadius:10,color:A.gold,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
      ...
      {backupDate&&(
        <div style={{background:A.sf2,borderRadius:10,padding:'10px 14px',marginBottom:10,display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8}}>
          <div>
            <div style={{fontSize:12,fontWeight:600,color:A.text}}>💾 Backup local disponible</div>
            <div style={{fontSize:11,color:A.muted,marginTop:2}}>Guardado: {backupDate}</div>
          </div>
          <button onClick={localBackup} style={{padding:'7px 14px',background:`${A.terra}15`,border:`1px solid ${A.terra}44`,borderRadius:8,color:A.terra,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>
      ...
      {restoreMsg&&(
        <div style={{padding:'10px 14px',borderRadius:10,background:restoreOk?`${A.teal}15`:`${A.terra}15`,color:restoreOk?A.teal:A.terra,fontSize:13,fontWeight:600}}>
          {restoreMsg}
        </div>
      )}

      <div style={{marginTop:12,fontSize:11,color:A.muted,lineHeight:1.7,background:A.sf2,borderRadius:8,padding:'10px 12px'}}>
        💡 <strong style={{color:A.text}}>Recomendación:</strong> Exporta un backup JSON después de cada jornada importante. 
        Guárdalo en Google Drive o iCloud para tenerlo seguro fuera del ordenador.
      </div>
    </>}/>
  );
}
