...
function CierreModal({appt,D,onClose,onDone,multiCount=1,multiTotal=0}){
  ...
  const basePrice=isMulti?multiTotal:appt.price;
...
function AdminConfig({D,commit}){
  const [form,setForm]=useState({googlePlaceId:(D.settings?.googlePlaceId)||'',googleApiKey:(D.settings?.googleApiKey)||'',googleClientId:(D.settings?.googleClientId)||''});
  const [saved,setSaved]=useState(false);
