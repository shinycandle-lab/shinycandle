        ...
        <div style={{padding:isMobile?'16px 20px 32px':'18px 26px 32px'}}>
          ...
          {step===3&&<div>
            ...
            <div style={{background:WEB.light,borderRadius:12,padding:'12px 16px',margin:'14px 0 18px',fontSize:12,color:WEB.muted,lineHeight:1.7}}>
              {method==='bizum'&&'📱 Envía el pago al +34 605 010 487 con tu nombre. Activaremos la tarjeta en menos de 24h.'}
          ...
          {step===4&&card&&<div>
            <div style={{textAlign:'center',marginBottom:20}}>
              ...
              <div style={{fontSize:13,color:WEB.muted,lineHeight:1.8}}>
                {card.method==='bizum'&&`Envía ${fmt(finalAmount)} por Bizum al +34 605 010 487 con tu nombre. Activaremos la tarjeta en menos de 24h.`}
...
function BookingModal({D,commit,onClose}){
  ...
  const getEffectivePrice=sv=>{
    if(!(sv.variants||[]).length)return sv.price;
    const vid=svcVariants[sv.id];
    const vr=(sv.variants||[]).find(v=>v.id===vid)||(sv.variants||[])[0];
    return vr?.price??sv.price;
  };
  const getVariantName=sv=>{
    if(!(sv.variants||[]).length)return null;
    const vid=svcVariants[sv.id];
    return ((sv.variants||[]).find(v=>v.id===vid)||(sv.variants||[])[0])?.name||null;
  };
  ...
  const toggleSvc=sv=>{
    setSelSvcs(prev=>{
      ...
      if(has)return prev.filter(x=>x.id!==sv.id);
      if((sv.variants||[]).length>0)
        setSvcVariants(p=>({...p,[sv.id]:sv.variants[0]?.id||null}));
      return [...prev,sv];
    });
    setTime('');
  };
