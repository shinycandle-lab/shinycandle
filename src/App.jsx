...
function AdminAgenda({D,commit,setModal,ask}){
  ...
  const cobrar=({method,sold,extraSvcs,tip,notes,total,giftCard})=>{
    ...
    const txs=[];
    let offset=0;
    // Si hay tarjeta regalo, registrar el uso de la tarjeta como ingreso especial
    if(giftUsed>0){
      txs.push({id:id0+offset,date:tod(),type:'income',category:'Tarjetas Regalo',description:`Canjeo tarjeta ${giftCard.code} — ${cierre.clientName}`,amount:giftUsed,method:'tarjeta_regalo',staffId:cierre.staffId});
      offset++;
    }
    // Si queda saldo a cobrar por otro método
    if(remainingTotal>0){
      txs.push({id:id0+offset,date:tod(),type:'income',category:'Servicios',description:`${cierre.serviceName} — ${cierre.clientName}${tip>0?` (+propina ${fmt(tip)})`:''}${giftUsed>0?' (resto tras tarjeta regalo)':''}`,amount:remainingTotal,method,staffId:cierre.staffId});
      offset++;
    } else if(giftUsed<=0){
      // Sin tarjeta regalo, registrar normalmente
...
function AdminTarjetas({D,commit,ask}){
  ...
  const createManual=()=>{
    ...
    commit({...D,giftCards:[...(D.giftCards||[]),created]});
    setShowCreate(false);
